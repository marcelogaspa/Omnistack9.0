...
import {  View, Text, StyleSheet, FlatList, Image, TouchableOpacity } from 'react-native';
 ...
 function SpotList({ techs, navigation }) {
    ...
    <View style={styles.container}>
        <Text style={style.title}>Companies use <Text style={styles.bold} />{tech}</Text>
        <FlatList
            style={style.list}
            data={spots}
            ...
            renderItem={({ item }) => (
                <View>
                    ...
                    <Text style={style.price}>{item.price ? `R$${item.price}/day` : 'FOR FREE!' } </Text>
                    <TouchableOpacity onPress={() => handleNavigate(item._id)} style={style.button}>
                        <Text style={style.buttonText}>Reserve</Text>
                    </TouchableOpacity>
